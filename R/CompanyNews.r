...
CompanyNews <- R6::R6Class(
  ...
  public = list(
    ...
    `body` = NA,
    initialize = function(`id`, `title`, `publication_date`, `url`, `summary`, `source`, `company`, `topics`, `copyright`, `language`, `word_count`, `spam`, `business_relevance`, `article_sentiment`, `article_sentiment_confidence`, `body`){
      if (!missing(`id`)) {
        self$`id` <- `id`
      }
      if (!missing(`title`)) {
    ...
    toJSON = function() {
      ...
      if (!is.null(self$`body`)) {
        ...
        if (is.list(self$`body`) && ((length(self$`body`) == 0) || ((length(self$`body`) != 0 && R6::is.R6(self$`body`[[1]]))))) {
          CompanyNewsObject[['body']] <- lapply(self$`body`, function(x) x$toJSON())
        } else {
          CompanyNewsObject[['body']] <- jsonlite::toJSON(self$`body`, auto_unbox = TRUE)
        }
      }
    ...
    fromJSON = function(CompanyNewsJson) {
      ...
      if (!is.null(CompanyNewsObject$`body`)) {
        self$`body` <- CompanyNewsObject$`body`
      }
    },
