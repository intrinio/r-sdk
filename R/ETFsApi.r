...
#'
#'
#' get_etf_historical_stats Exchange Traded Fund (ETF) Historical Stats
#'
#'
...
ETFsApi <- R6::R6Class(
  ...
  public = list(
    ...
    get_etf_historical_stats = function(identifier, opts = list()){
      queryParams <- list()
      headerParams <- character()
      ...
      urlPath <- "/etfs/{identifier}/historical_stats"
      if (!missing(`identifier`)) {
        urlPath <- gsub(paste0("\\{", "identifier", "\\}"), `identifier`, urlPath)
      }
      ...
      if (httr::status_code(resp) >= 200 && httr::status_code(resp) <= 299) {
        ...
        if ("ETFHistoricalStats" == "Numeric") {
          returnObject <- as.numeric(jsonlite::fromJSON(result))
        } else if ("ETFHistoricalStats" == "Integer") {
          returnObject <- as.integer(jsonlite::fromJSON(result))
        } else if ("ETFHistoricalStats" == "Character") {
          returnObject <- gsub("\\\"", "", result)
        } else if ("ETFHistoricalStats" == "Logical") {
          returnObject <- as.logical(jsonlite::fromJSON(result))
        } else {
          returnObject <- ETFHistoricalStats$new()
          returnObject$fromJSONString(result)
        }
