...
EconomicIndex <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(EconomicIndexJson) {
      ...
      self$`last_updated` <- POSIXltObject$fromJSON(jsonlite::toJSON(EconomicIndexObject$last_updated, auto_unbox = TRUE))
      self$`description` <- EconomicIndexObject$`description`
      DateObject <- Date$new()
      self$`observation_start` <- DateObject$fromJSON(jsonlite::toJSON(EconomicIndexObject$observation_start, auto_unbox = TRUE))
      DateObject <- Date$new()
      self$`observation_end` <- DateObject$fromJSON(jsonlite::toJSON(EconomicIndexObject$observation_end, auto_unbox = TRUE))
      self$`seasonal_adjustment` <- EconomicIndexObject$`seasonal_adjustment`
      self$`units` <- EconomicIndexObject$`units`
    }
  )
