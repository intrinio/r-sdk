...
VolumePriceTrendTechnicalValue <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`date_time`, `vpt`){
      ...
      if (!missing(`vpt`)) {
        stopifnot(is.numeric(`vpt`), length(`vpt`) == 1)
        self$`vpt` <- `vpt`
      }
    },
    toJSON = function() {
    ...
    fromJSONString = function(VolumePriceTrendTechnicalValueJson) {
      ...
      self$`date_time` <- POSIXltObject$fromJSON(jsonlite::toJSON(VolumePriceTrendTechnicalValueObject$date_time, auto_unbox = TRUE))
      self$`vpt` <- VolumePriceTrendTechnicalValueObject$`vpt`
    }
  )
)
