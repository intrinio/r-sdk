...
ChaikinMoneyFlowTechnicalValue <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`date_time`, `cmf`){
      ...
      if (!missing(`cmf`)) {
        stopifnot(is.numeric(`cmf`), length(`cmf`) == 1)
        self$`cmf` <- `cmf`
      }
    },
    toJSON = function() {
    ...
    fromJSONString = function(ChaikinMoneyFlowTechnicalValueJson) {
      ...
      self$`date_time` <- POSIXltObject$fromJSON(jsonlite::toJSON(ChaikinMoneyFlowTechnicalValueObject$date_time, auto_unbox = TRUE))
      self$`cmf` <- ChaikinMoneyFlowTechnicalValueObject$`cmf`
    }
  )
)
