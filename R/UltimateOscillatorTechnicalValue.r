...
UltimateOscillatorTechnicalValue <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`date_time`, `uo`){
      ...
      if (!missing(`uo`)) {
        stopifnot(is.numeric(`uo`), length(`uo`) == 1)
        self$`uo` <- `uo`
      }
    },
    toJSON = function() {
    ...
    fromJSONString = function(UltimateOscillatorTechnicalValueJson) {
      ...
      self$`date_time` <- POSIXltObject$fromJSON(jsonlite::toJSON(UltimateOscillatorTechnicalValueObject$date_time, auto_unbox = TRUE))
      self$`uo` <- UltimateOscillatorTechnicalValueObject$`uo`
    }
  )
)
