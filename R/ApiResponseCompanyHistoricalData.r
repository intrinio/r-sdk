...
ApiResponseCompanyHistoricalData <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(ApiResponseCompanyHistoricalDataJson) {
      ApiResponseCompanyHistoricalDataObject <- jsonlite::fromJSON(ApiResponseCompanyHistoricalDataJson)
      self$`historical_data` <- lapply(ApiResponseCompanyHistoricalDataObject$`historical_data`, function(x) HistoricalData$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
      CompanySummaryObject <- CompanySummary$new()
      self$`company` <- CompanySummaryObject$fromJSON(jsonlite::toJSON(ApiResponseCompanyHistoricalDataObject$company, auto_unbox = TRUE))
      self$`next_page` <- ApiResponseCompanyHistoricalDataObject$`next_page`
    }
  )
)
