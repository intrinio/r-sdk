...
AverageDirectionalIndexTechnicalValue <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`date_time`, `adx`, `di_neg`, `di_pos`){
      ...
      if (!missing(`adx`)) {
        stopifnot(is.numeric(`adx`), length(`adx`) == 1)
        self$`adx` <- `adx`
      }
      if (!missing(`di_neg`)) {
        stopifnot(is.numeric(`di_neg`), length(`di_neg`) == 1)
        self$`di_neg` <- `di_neg`
      }
      if (!missing(`di_pos`)) {
        stopifnot(is.numeric(`di_pos`), length(`di_pos`) == 1)
        self$`di_pos` <- `di_pos`
      }
    },
    toJSON = function() {
    ...
    fromJSONString = function(AverageDirectionalIndexTechnicalValueJson) {
      ...
      self$`date_time` <- POSIXltObject$fromJSON(jsonlite::toJSON(AverageDirectionalIndexTechnicalValueObject$date_time, auto_unbox = TRUE))
      self$`adx` <- AverageDirectionalIndexTechnicalValueObject$`adx`
      self$`di_neg` <- AverageDirectionalIndexTechnicalValueObject$`di_neg`
      self$`di_pos` <- AverageDirectionalIndexTechnicalValueObject$`di_pos`
    }
