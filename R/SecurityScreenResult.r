...
SecurityScreenResult <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(SecurityScreenResultJson) {
      ...
      self$`security` <- SecuritySummaryObject$fromJSON(jsonlite::toJSON(SecurityScreenResultObject$security, auto_unbox = TRUE))
      self$`data` <- lapply(SecurityScreenResultObject$`data`, function(x) SecurityScreenResultData$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
    }
  )
)
