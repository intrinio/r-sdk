...
ApiResponseSecurityHistoricalData <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(ApiResponseSecurityHistoricalDataJson) {
      ApiResponseSecurityHistoricalDataObject <- jsonlite::fromJSON(ApiResponseSecurityHistoricalDataJson)
      self$`historical_data` <- lapply(ApiResponseSecurityHistoricalDataObject$`historical_data`, function(x) HistoricalData$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
      SecuritySummaryObject <- SecuritySummary$new()
      self$`security` <- SecuritySummaryObject$fromJSON(jsonlite::toJSON(ApiResponseSecurityHistoricalDataObject$security, auto_unbox = TRUE))
      self$`next_page` <- ApiResponseSecurityHistoricalDataObject$`next_page`
    }
  )
)
