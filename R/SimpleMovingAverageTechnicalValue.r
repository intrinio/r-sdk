...
SimpleMovingAverageTechnicalValue <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`date_time`, `sma`){
      ...
      if (!missing(`sma`)) {
        stopifnot(is.numeric(`sma`), length(`sma`) == 1)
        self$`sma` <- `sma`
      }
    },
    toJSON = function() {
    ...
    fromJSONString = function(SimpleMovingAverageTechnicalValueJson) {
      ...
      self$`date_time` <- POSIXltObject$fromJSON(jsonlite::toJSON(SimpleMovingAverageTechnicalValueObject$date_time, auto_unbox = TRUE))
      self$`sma` <- SimpleMovingAverageTechnicalValueObject$`sma`
    }
  )
)
