...
TheaSourceDocument <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`id`, `date_time`, `tags`){
      if (!missing(`id`)) {
        self$`id` <- `id`
      }
    ...
    toJSON = function() {
      ...
      if (!is.null(self$`date_time`)) {
        ...
        if (is.list(self$`date_time`) && ((length(self$`date_time`) == 0) || ((length(self$`date_time`) != 0 && R6::is.R6(self$`date_time`[[1]]))))) {
          TheaSourceDocumentObject[['date_time']] <- lapply(self$`date_time`, function(x) x$toJSON())
        } else {
          TheaSourceDocumentObject[['date_time']] <- jsonlite::toJSON(self$`date_time`, auto_unbox = TRUE)
        }
      }
    ...
    fromJSON = function(TheaSourceDocumentJson) {
      ...
      if (!is.null(TheaSourceDocumentObject$`date_time`)) {
        self$`date_time` <- TheaSourceDocumentObject$`date_time`
      }
    ...
    setFromList = function(listObject) {
      ...
      if (!is.null(listObject$`date_time`)) {
        self$`date_time` <- self$`date_time` <- as.Date(listObject$`date_time`, "%Y-%m-%d")
      }
      else {
        self$`date_time` <- NA 
      }
