...
ApiResponseOptionPrices <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(ApiResponseOptionPricesJson) {
      ApiResponseOptionPricesObject <- jsonlite::fromJSON(ApiResponseOptionPricesJson)
      self$`prices` <- lapply(ApiResponseOptionPricesObject$`prices`, function(x) OptionPrice$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
      OptionObject <- Option$new()
      self$`option` <- OptionObject$fromJSON(jsonlite::toJSON(ApiResponseOptionPricesObject$option, auto_unbox = TRUE))
      self$`next_page` <- ApiResponseOptionPricesObject$`next_page`
    }
  )
)
