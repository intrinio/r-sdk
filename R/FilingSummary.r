...
FilingSummary <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(FilingSummaryJson) {
      ...
      self$`id` <- FilingSummaryObject$`id`
      DateObject <- Date$new()
      self$`filing_date` <- DateObject$fromJSON(jsonlite::toJSON(FilingSummaryObject$filing_date, auto_unbox = TRUE))
      POSIXltObject <- POSIXlt$new()
      self$`accepted_date` <- POSIXltObject$fromJSON(jsonlite::toJSON(FilingSummaryObject$accepted_date, auto_unbox = TRUE))
      DateObject <- Date$new()
      self$`period_end_date` <- DateObject$fromJSON(jsonlite::toJSON(FilingSummaryObject$period_end_date, auto_unbox = TRUE))
      self$`report_type` <- FilingSummaryObject$`report_type`
      self$`sec_unique_id` <- FilingSummaryObject$`sec_unique_id`
      self$`filing_url` <- FilingSummaryObject$`filing_url`
      self$`report_url` <- FilingSummaryObject$`report_url`
