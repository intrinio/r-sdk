...
MoneyFlowIndexTechnicalValue <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`date_time`, `mfi`){
      ...
      if (!missing(`mfi`)) {
        stopifnot(is.numeric(`mfi`), length(`mfi`) == 1)
        self$`mfi` <- `mfi`
      }
    },
    toJSON = function() {
    ...
    fromJSONString = function(MoneyFlowIndexTechnicalValueJson) {
      ...
      self$`date_time` <- POSIXltObject$fromJSON(jsonlite::toJSON(MoneyFlowIndexTechnicalValueObject$date_time, auto_unbox = TRUE))
      self$`mfi` <- MoneyFlowIndexTechnicalValueObject$`mfi`
    }
  )
)
