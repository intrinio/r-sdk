...
VolumeWeightedAveragePriceValue <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`date_time`, `vwap`){
      ...
      if (!missing(`vwap`)) {
        stopifnot(is.numeric(`vwap`), length(`vwap`) == 1)
        self$`vwap` <- `vwap`
      }
    },
    toJSON = function() {
    ...
    fromJSONString = function(VolumeWeightedAveragePriceValueJson) {
      ...
      self$`date_time` <- POSIXltObject$fromJSON(jsonlite::toJSON(VolumeWeightedAveragePriceValueObject$date_time, auto_unbox = TRUE))
      self$`vwap` <- VolumeWeightedAveragePriceValueObject$`vwap`
    }
  )
)
