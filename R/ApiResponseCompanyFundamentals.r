...
ApiResponseCompanyFundamentals <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(ApiResponseCompanyFundamentalsJson) {
      ApiResponseCompanyFundamentalsObject <- jsonlite::fromJSON(ApiResponseCompanyFundamentalsJson)
      self$`fundamentals` <- lapply(ApiResponseCompanyFundamentalsObject$`fundamentals`, function(x) FundamentalSummary$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
      CompanySummaryObject <- CompanySummary$new()
      self$`company` <- CompanySummaryObject$fromJSON(jsonlite::toJSON(ApiResponseCompanyFundamentalsObject$company, auto_unbox = TRUE))
      self$`next_page` <- ApiResponseCompanyFundamentalsObject$`next_page`
    }
  )
)
