...
OptionIntervalMover <- R6::R6Class(
  ...
  public = list(
    ...
    `average` = NA,
    `trade_count` = NA,
    initialize = function(`contract`, `change`, `open`, `high`, `low`, `close`, `volume`, `average`, `trade_count`){
      if (!missing(`contract`)) {
    ...
    toJSON = function() {
      ...
      if (!is.null(self$`trade_count`)) {
        ...
        if (is.list(self$`trade_count`) && ((length(self$`trade_count`) == 0) || ((length(self$`trade_count`) != 0 && R6::is.R6(self$`trade_count`[[1]]))))) {
          OptionIntervalMoverObject[['trade_count']] <- lapply(self$`trade_count`, function(x) x$toJSON())
        } else {
          OptionIntervalMoverObject[['trade_count']] <- jsonlite::toJSON(self$`trade_count`, auto_unbox = TRUE)
        }
      }
    ...
    fromJSON = function(OptionIntervalMoverJson) {
      ...
      if (!is.null(OptionIntervalMoverObject$`trade_count`)) {
        self$`trade_count` <- OptionIntervalMoverObject$`trade_count`
      }
    },
