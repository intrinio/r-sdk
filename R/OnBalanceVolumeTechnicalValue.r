...
OnBalanceVolumeTechnicalValue <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`date_time`, `obv`){
      ...
      if (!missing(`obv`)) {
        stopifnot(is.numeric(`obv`), length(`obv`) == 1)
        self$`obv` <- `obv`
      }
    },
    toJSON = function() {
    ...
    fromJSONString = function(OnBalanceVolumeTechnicalValueJson) {
      ...
      self$`date_time` <- POSIXltObject$fromJSON(jsonlite::toJSON(OnBalanceVolumeTechnicalValueObject$date_time, auto_unbox = TRUE))
      self$`obv` <- OnBalanceVolumeTechnicalValueObject$`obv`
    }
  )
)
