...
StockMarketIndex <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(StockMarketIndexJson) {
      ...
      self$`last_updated` <- POSIXltObject$fromJSON(jsonlite::toJSON(StockMarketIndexObject$last_updated, auto_unbox = TRUE))
      self$`description` <- StockMarketIndexObject$`description`
      DateObject <- Date$new()
      self$`observation_start` <- DateObject$fromJSON(jsonlite::toJSON(StockMarketIndexObject$observation_start, auto_unbox = TRUE))
      DateObject <- Date$new()
      self$`observation_end` <- DateObject$fromJSON(jsonlite::toJSON(StockMarketIndexObject$observation_end, auto_unbox = TRUE))
    }
  )
)
