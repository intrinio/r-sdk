...
ApiResponseSecurityVortexIndicator <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(ApiResponseSecurityVortexIndicatorJson) {
      ApiResponseSecurityVortexIndicatorObject <- jsonlite::fromJSON(ApiResponseSecurityVortexIndicatorJson)
      self$`technicals` <- lapply(ApiResponseSecurityVortexIndicatorObject$`technicals`, function(x) VortexIndicatorTechnicalValue$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
      TechnicalIndicatorObject <- TechnicalIndicator$new()
      self$`indicator` <- TechnicalIndicatorObject$fromJSON(jsonlite::toJSON(ApiResponseSecurityVortexIndicatorObject$indicator, auto_unbox = TRUE))
      SecuritySummaryObject <- SecuritySummary$new()
      self$`security` <- SecuritySummaryObject$fromJSON(jsonlite::toJSON(ApiResponseSecurityVortexIndicatorObject$security, auto_unbox = TRUE))
      self$`next_page` <- ApiResponseSecurityVortexIndicatorObject$`next_page`
    }
  )
)
