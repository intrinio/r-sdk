...
ApiResponseSICIndexHistoricalData <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(ApiResponseSICIndexHistoricalDataJson) {
      ApiResponseSICIndexHistoricalDataObject <- jsonlite::fromJSON(ApiResponseSICIndexHistoricalDataJson)
      self$`historical_data` <- lapply(ApiResponseSICIndexHistoricalDataObject$`historical_data`, function(x) HistoricalData$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
      SICIndexObject <- SICIndex$new()
      self$`index` <- SICIndexObject$fromJSON(jsonlite::toJSON(ApiResponseSICIndexHistoricalDataObject$index, auto_unbox = TRUE))
      self$`next_page` <- ApiResponseSICIndexHistoricalDataObject$`next_page`
    }
  )
)
