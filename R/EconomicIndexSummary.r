  ...
  public = list(
    ...
    initialize = function(`id`, `symbol`, `name`, `continent`, `country`, `update_frequency`, `last_updated`, `observation_start`, `observation_end`, `seasonal_adjustment`, `units`){
      ...
      if (!missing(`seasonal_adjustment`)) {
        stopifnot(is.character(`seasonal_adjustment`), length(`seasonal_adjustment`) == 1)
    ...
    fromJSONString = function(EconomicIndexSummaryJson) {
      ...
      self$`last_updated` <- POSIXltObject$fromJSON(jsonlite::toJSON(EconomicIndexSummaryObject$last_updated, auto_unbox = TRUE))
      DateObject <- Date$new()
      self$`observation_start` <- DateObject$fromJSON(jsonlite::toJSON(EconomicIndexSummaryObject$observation_start, auto_unbox = TRUE))
      DateObject <- Date$new()
      self$`observation_end` <- DateObject$fromJSON(jsonlite::toJSON(EconomicIndexSummaryObject$observation_end, auto_unbox = TRUE))
      self$`seasonal_adjustment` <- EconomicIndexSummaryObject$`seasonal_adjustment`
      self$`units` <- EconomicIndexSummaryObject$`units`
    }
  )
