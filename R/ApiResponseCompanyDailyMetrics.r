...
ApiResponseCompanyDailyMetrics <- R6::R6Class(
  ...
  public = list(
    ...
    `company` = NA,
    `next_page` = NA,
    initialize = function(`daily_metrics`, `company`, `next_page`){
    ...
    toJSON = function() {
      ...
      if (!is.null(self$`company`)) {
        ...
        if (is.list(self$`company`) && ((length(self$`company`) == 0) || ((length(self$`company`) != 0 && R6::is.R6(self$`company`[[1]]))))) {
          ApiResponseCompanyDailyMetricsObject[['company']] <- lapply(self$`company`, function(x) x$toJSON())
        } else {
          ApiResponseCompanyDailyMetricsObject[['company']] <- jsonlite::toJSON(self$`company`, auto_unbox = TRUE)
        }
      }
    ...
    fromJSON = function(ApiResponseCompanyDailyMetricsJson) {
      ...
      if (!is.null(ApiResponseCompanyDailyMetricsObject$`company`)) {
        self$`company` <- ApiResponseCompanyDailyMetricsObject$`company`
      }
