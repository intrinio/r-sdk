...
Filing <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSONString = function(FilingJson) {
      ...
      self$`id` <- FilingObject$`id`
      DateObject <- Date$new()
      self$`filing_date` <- DateObject$fromJSON(jsonlite::toJSON(FilingObject$filing_date, auto_unbox = TRUE))
      POSIXltObject <- POSIXlt$new()
      self$`accepted_date` <- POSIXltObject$fromJSON(jsonlite::toJSON(FilingObject$accepted_date, auto_unbox = TRUE))
      DateObject <- Date$new()
      self$`period_end_date` <- DateObject$fromJSON(jsonlite::toJSON(FilingObject$period_end_date, auto_unbox = TRUE))
      self$`report_type` <- FilingObject$`report_type`
      self$`sec_unique_id` <- FilingObject$`sec_unique_id`
      self$`filing_url` <- FilingObject$`filing_url`
      self$`report_url` <- FilingObject$`report_url`
      ...
      self$`company` <- CompanySummaryObject$fromJSON(jsonlite::toJSON(FilingObject$company, auto_unbox = TRUE))
    }
  )
)
