...
ApiResponseSecurityZacksAnalystRatings <- R6::R6Class(
  ...
  public = list(
    `analyst_ratings` = NULL,
    `security` = NULL,
    `next_page` = NULL,
    initialize = function(`analyst_ratings`, `security`, `next_page`){
      if (!missing(`analyst_ratings`)) {
        stopifnot(is.list(`analyst_ratings`))
        lapply(`analyst_ratings`, function(x) stopifnot(R6::is.R6(x)))
    ...
    fromJSONString = function(ApiResponseSecurityZacksAnalystRatingsJson) {
      ApiResponseSecurityZacksAnalystRatingsObject <- jsonlite::fromJSON(ApiResponseSecurityZacksAnalystRatingsJson)
      self$`analyst_ratings` <- lapply(ApiResponseSecurityZacksAnalystRatingsObject$`analyst_ratings`, function(x) ZacksAnalystRatingSummary$new()$fromJSON(jsonlite::toJSON(x, auto_unbox = TRUE)))
      SecuritySummaryObject <- SecuritySummary$new()
      self$`security` <- SecuritySummaryObject$fromJSON(jsonlite::toJSON(ApiResponseSecurityZacksAnalystRatingsObject$security, auto_unbox = TRUE))
      self$`next_page` <- ApiResponseSecurityZacksAnalystRatingsObject$`next_page`
    }
  )
)
